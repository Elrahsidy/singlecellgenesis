    ...
    for (i=1;i<=(P5IBd_NY);i=i+1)
        ...
        for (j=1;j<=(P5IBd_NX);j=j+1)
            ...
            if ( {randneur <= neuronfrac} )
                ...
                if ({output}==1)
                    echo P5IBd {mynode} {i} {j} is receiving random input (randneur=={randneur}<={neuronfrac}) at {Ranrate} Hz
                end
                ...
                setfield /P5IBdnet/P5IBd[{dex}]/apdend3/Ex_chSPIKEAMPA synapse[0].delay 0 synapse[0].weight 1
