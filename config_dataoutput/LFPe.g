...
float LFPz = 3000e-6 // just above column surface

// ayu: Put an LFP object over each column
for (j=0; j<numelecs; j=j+1)
     create efield LFP{j}
     setfield LFP{j} scale 0.335 x {(j % sqrtNnodes)*NX*SEPX} y {(j / sqrtNnodes)*NY*SEPY} z {LFPz}
     echo Position LFP {j} NA NA {(j % sqrtNnodes)*NX*SEPX} {(j / sqrtNnodes)*NY*SEPY} {LFPz}
end
